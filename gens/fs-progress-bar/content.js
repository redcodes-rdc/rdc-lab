const generatorContent = {
  leftView: {
    ...
    description:
      "Create a Shopify-focused free shipping progress bar with dynamic JavaScript and threshold controls. Use it in cart drawers, cart pages, popups, or custom sections, then refine with AI prompts.",
    callouts: [
      {
    ...
    useCases: [
      ...
      "Show threshold progress on Shopify cart pages",
      "Display shipping incentives inside modals or popups",
      "Add cart-value messaging to custom Shopify sections",
    ],
    useCta: {
...
`,
      text: "Create dynamic free shipping bars that update as customers shop.",
    },
    output: {
      element: "pre",
      codeId: "fsOutput",
  ...
  settings: {
        ...
        fields: [
          {
            ...
            id: "fs-threshold",
            className: "fs-threshold",
            placeholder: "50",
            value: "50",
          },
          {
            type: "number",
            label: "Current Amount (Demo)",
            id: "fs-current",
            className: "fs-current",
            placeholder: "20",
            value: "20",
          },
        ],
    ...
    generatorCssDownload: {
      href: "",
      fileName: "free-shipping-bar-generator.css",
...
`,
  ...
  preview: {
    ...
    main: {
      id: "fsPreview",
      className: "rlab-fs-main",
      innerClass: "rlab-fs-inner",
      message: {
        id: "fsMessage",
        className: "rlab-fs-message",
      },
      bar: {
        className: "rlab-fs-bar rdc-w-full",
        fill: {
          id: "fsBarFill",
          className: "rlab-fs-bar-fill",
        },
      },
    },
  },
...
initFreeShippingBarGenerator(generatorContent);

function initFreeShippingBarGenerator(content) {
  const init = () => {
    const hasTemplate =
    ...
    bindFreeShippingBarGenerator(content);
  };
...
function renderFieldControl(field) {
  return `<input type="${field.type}" id="${field.id}" class="${field.className} rdcl-gen--cols-field rdc-w-half" placeholder="${field.placeholder}" value="${field.value || ""}" />`;
}
...
function renderPreview(content) {
  ...
  return `
    <div class="${content.wrapperClass}">
      <div class="${main.className}" id="${main.id}">
        <div class="${main.innerClass}">
          <p class="${main.message.className}" id="${main.message.id}"></p>

          <div class="${main.bar.className}">
            <div class="${main.bar.fill.className}" id="${main.bar.fill.id}"></div>
          </div>
        </div>
      </div>
    </div>
...
function bindFreeShippingBarGenerator(content) {
  const thresholdInput = document.querySelector("#fs-threshold");
  const currentInput = document.querySelector("#fs-current");
  ...
  copyButton.onclick = copyOutputCode;
  copyButton.addEventListener("click", copyOutputCode, true);

  update();
}

function getFreeShippingState() {
  const threshold = getNumberValue("fs-threshold");
  const current = getNumberValue("fs-current");
  const remaining = Math.max(threshold - current, 0);
  const progress = calculateProgress(threshold, current);
  const message = getFreeShippingMessage(threshold, current);
...
function updatePreview(previewContainer) {
  ...
  const message = document.querySelector("#fsMessage");
  const barFill = document.querySelector("#fsBarFill");
...
function updatePreviewStyles() {
  let style = document.querySelector("#fs-preview-style");

  if (!style) {
    style = document.createElement("style");
    style.id = "fs-preview-style";
...
function buildFreeShippingOutput() {
  const state = getFreeShippingState();
...
(function () {
  ...
  const currentAmount = ${state.current || 0};
...
function getFreeShippingMessage(threshold, current) {
  ...
  return `You're $${formatAmount(threshold - current)} away from free shipping`;
}

function getNumberValue(id) {
  const control = document.querySelector(`#${id}`);
  const value = parseFloat(control.value);
  const placeholder = parseFloat(control.placeholder);
