const tooltipContent = {
  leftView: {
    ...
    description:
      "Generate lightweight CSS tooltips for product details, form help, pricing notes, and UI guidance. Customize position, style, alignment, and radius, then refine accessibility with AI prompts.",
    callouts: [
      {
    ...
    useCases: [
      "Explain product features, ingredients, or specifications",
      "Add helper text to forms, inputs, and settings",
      "Display size guide, fit, or care information",
      "Provide additional details for pricing, plans, or limits",
      "Explain shipping, delivery, or policy details",
      "Add context to product customization choices",
    ],
    useCta: {
...
`,
      text: "Create clean hover tooltips for products, pricing or other elements.",
    },
    outputId: "tipOutput",
  },
  settings: {
    fields: [
      {
        ...
        id: "tip-style",
        options: [
          { value: "bubble", label: "Bubble", selected: true },
      ...
      },
      {
        ...
        id: "tip-position",
        options: [
          { value: "top", label: "Top", selected: true },
    ...
    generatorCssDownload: {
      href: "",
      fileName: "tooltip-generator.css",
      label: `Download This Generator's CSS File`,
    },
    globalCssDownload: {
      href: "/assets/css/global.css",
      fileName: "rdc-lab-global.css",
      label: `Download Global CSS File`,
    },
  },
  previewStageId: "tipPreviewStage",
  tutorial: {
...
function renderTooltipContent(content) {
  const leftContainer = document.querySelector(".rdcl-gen--cols-vw");
  const settingsContainer = document.querySelector(".rdcl-gen--cols-set");
  const previewContainer = document.querySelector(".rdcl-gen--layout-vw");

  if (!leftContainer || !settingsContainer || !previewContainer) return;

  leftContainer.innerHTML = renderTooltipLeftView(content.leftView);
  settingsContainer.innerHTML = renderTooltipSettings(content.settings);
  previewContainer.innerHTML = `
    <div class="rdcl-gen--layout-vw-inner" id="${content.previewStageId}"></div>
  `;
  insertTutorial(content.tutorial);
  insertSmallCta(content.smallCta);
}

function renderTooltipLeftView(content) {
  return `
...
function renderTooltipSettings(content) {
  return `
    <div class="rdcl-gen--cols-set-inner">
      <div class="rdc-d-flex rdc-ff-col">
        <h4>Settings</h4>
        ${content.fields.map(renderTooltipField).join("")}
        <button id="rdcl-copy-btn" type="button">Copy Code</button>
...
function renderCssDownloads(content) {
  return [content.generatorCssDownload, content.globalCssDownload]
    .filter((download) => download && download.href)
    .map(renderCssDownload)
    .join("");
}

function renderCssDownload(download) {
  return `<a class="rdcl-copy-css-btn rdc-d-none rdc-fs-12 rdc-c-white-2 rdc-jc-cen rdc-m-t10 rdc-ff-baij" href="${download.href}" download="${download.fileName || ""}">${download.label}</a>`;
}

function renderTooltipField(field) {
  return `
    <div class="rdc-d-flex">
      <label class="rdc-w-half" for="${field.id}">${field.label}</label>
      ${renderTooltipControl(field)}
    </div>
  `;
}

function renderTooltipControl(field) {
  if (field.type === "select") {
    return `
      <select id="${field.id}" class="rdcl-gen--cols-field rdc-w-half">
        ${field.options.map(renderTooltipOption).join("")}
      </select>
    `;
  }
  ...
  return `
    <input
      type="${field.type}"
      id="${field.id}"
      class="rdcl-gen--cols-field rdc-w-half"
      placeholder="${field.placeholder}"
    />
  `;
}

function renderTooltipOption(option) {
  return `
