const generatorContent = {
  leftView: {
    title: "HTML Column Layout Generator",
    description:
      "Build responsive multi-column HTML/CSS layouts for cards, features, categories, services, and content grids. Control wrapping and device behavior, then refine the code with AI prompts.",
    callouts: [
      {
        icon: `<img src="/assets/icons/overline-co-icon.svg" alt="">`,
        text: "Responsive Grid",
      },
      {
        icon: `<img src="/assets/icons/overline-co-icon.svg" alt="">`,
        text: "Device Controls",
      },
      {
        icon: `<img src="/assets/icons/overline-co-icon.svg" alt="">`,
        text: "Production Ready",
      },
    ],
    useCases: [
      "Display product, service, or feature cards in columns",
      "Create reusable multi-column content sections",
      "Showcase benefits in equal or custom-width columns",
      "Build team, partner, or logo grids",
      "Organize categories, collections, or resources",
      "Create responsive cards for landing pages and Shopify sections",
    ],
    useCta: {
      icon: `<svg width="151" height="151" viewBox="0 0 151 151" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
<rect x="2" y="2" width="146.766" height="146.766" rx="18" fill="#201D13" stroke="#C8FA33" stroke-width="4"/>
<rect x="22.0959" y="22.0959" width="106.574" height="106.574" fill="url(#pattern0_9916_1959)"/>
<defs>
<pattern id="pattern0_9916_1959" patternContentUnits="objectBoundingBox" width="1" height="1">
<use xlink:href="#image0_9916_1959" transform="scale(0.00390625)"/>
</pattern>
<image id="image0_9916_1959" width="256" height="256" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
</defs>
</svg>
`,
      text: "Build professional layouts that adapt perfectly across all devices.",
    },
    output: {
      element: "pre",
      codeId: "colsOutput",
      code: "",
    },
  },
  settings: {
    sections: [
      {
        fields: [
          {
            type: "number",
            label: "Column Count",
            className: "rdcl-gen--cols-count",
            value: "2",
            min: "2",
            max: "10",
          },
          {
            type: "number",
            label: "Columns per row",
            className: "rdcl-gen--cols-perrow",
            min: "2",
            max: "6",
          },
          {
            type: "number",
            label: "Scrollable Column Width",
            className: "rdcl-gen--cols-scrwidth",
            min: "200",
          },
        ],
      },
      {
        title: "Tablet",
        fields: [
          {
            type: "number",
            label: "Columns per row",
            className: "rdcl-gen--cols-perrowtab",
            min: "2",
            max: "6",
          },
          {
            type: "number",
            label: "Scrollable Column Width",
            className: "rdcl-gen--cols-scrwidthtab",
            min: "200",
          },
        ],
      },
      {
        title: "Mobile",
        fields: [
          {
            type: "number",
            label: "Columns per row",
            className: "rdcl-gen--cols-perrowmob",
            min: "2",
            max: "6",
          },
          {
            type: "number",
            label: "Scrollable Column Width",
            className: "rdcl-gen--cols-scrwidthmob",
            min: "200",
          },
        ],
      },
    ],
    actions: [
      {
        label: "Copy Code",
        id: "rdcl-copy-btn",
        type: "button",
      },
    ],
    generatorCssDownload: {
      href: "",
      fileName: "html-column-layout-generator.css",
      labelHtml: "Download This Generator's CSS File",
    },
    globalCssDownload: {
      href: "/assets/css/global.css",
      fileName: "global.css",
      labelHtml: "Download Global CSS File",
    },
  },
  tutorial: {
    projectPreview: `<img src="" alt="">`,
    video: "Tutorial video",
    text: "Follow the walkthrough to choose widths, set responsive behavior, preview the result, and copy the generated layout code.",
  },
  smallCta: {
    icon: `<svg width="151" height="151" viewBox="0 0 151 151" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
<rect x="2" y="2" width="146.766" height="146.766" rx="19.2521" fill="#1A1119" stroke="#51C4D1" stroke-width="4"/>
<mask id="mask0_9917_1997" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="12" y="12" width="127" height="127">
<rect x="12.1504" y="12.1504" width="126.465" height="126.465" fill="url(#pattern0_9917_1997)"/>
</mask>
<g mask="url(#mask0_9917_1997)">
<rect x="6.86816" y="6.86768" width="137.031" height="137.031" fill="#51C4D1"/>
</g>
<defs>
<pattern id="pattern0_9917_1997" patternContentUnits="objectBoundingBox" width="1" height="1">
<use xlink:href="#image0_9917_1997" transform="scale(0.00195312)"/>
</pattern>
<image id="image0_9917_1997" width="512" height="512" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
</defs>
</svg>
`,
    text: "All code is clean, lightweight, and 100% free to use in your projects.",
    button: {
      label: "Browse Generators",
      href: "/pages/html-generators/",
    },
  },
  preview: {
    wrapperClass: "rdcl-gen--layout-vw-inner",
    stageId: "colsPreviewStage",
    mainClass: "rlab-cols-main",
    innerClass: "rlab-cols-inner rdc-d-flex",
    columnClass: "rlab-cols-col",
  },
};

initColumnLayoutGenerator(generatorContent);

function initColumnLayoutGenerator(content) {
  const init = () => {
    const hasTemplate =
      document.querySelector(".rdcl-gen--cols-vw") &&
      document.querySelector(".rdcl-gen--cols-set") &&
      document.querySelector(".rdcl-gen--layout-vw");

    if (!hasTemplate) {
      requestAnimationFrame(init);
      return;
    }

    insertLeftView(content.leftView);
    insertSettings(content.settings);
    insertPreview(content.preview);
    insertTutorial(content.tutorial);
    insertSmallCta(content.smallCta);
    bindColumnLayoutGenerator(content);
  };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", init);
    return;
  }

  init();
}

function insertLeftView(content) {
  const container = document.querySelector(".rdcl-gen--cols-vw");

  if (!container) return;

  container.innerHTML = renderLeftView(content);
}

function renderLeftView(content) {
  return `
    <div class="rdcl-gen--cols-vw-inner">
      ${renderCallouts(content.callouts)}
      <h1>${content.title}</h1>
      <p>${content.description}</p>
      ${renderUseCases(content.useCases)}
      ${renderUseCta(content.useCta)}
      <${content.output.element}><code${content.output.codeId ? ` id="${content.output.codeId}"` : ""}>${escapeHtml(content.output.code || "")}</code></${content.output.element}>
    </div>
  `;
}

function renderCallouts(callouts = []) {
  if (!callouts.length) return "";

  return `
    <div class="rdcl-gen-co">
      <div class="rdcl-gen-co-inner">
        ${callouts.map(renderCallout).join("")}
      </div>
    </div>
  `;
}

function renderCallout(callout) {
  return `
    <span class="rdcl-gen-co-item rdc-lab-border-4 rdc-p-5 rdc-m-r5 rdc-bg-dark-6 rdc-d-iflex rdc-ai-cen">
      <span class="rdcl-gen-co-icon rdc-d-flex rdc-m-r5">${callout.icon}</span>
      <span class="rdcl-gen-co-text rdc-fs-10 rdc-tt-upc rdc-ff-baij rdc-lh-1">${callout.text}</span>
    </span>
  `;
}

function renderUseCases(useCases = []) {
  if (!useCases.length) return "";

  return `
    <div class="rdcl-gen-use rdc-m-bl20"> 
      <div class="rdcl-gen-use-inner rdc-lab-border rdc-p-20 rdc-bg-dark-6">
      <span class="rdc-ff-baij rdc-fw-500 rdc-c-white-3 rdc-d-flex rdc-ai-cen"><span class="rdcl-mini-h-icon rdc-d-iflex rdc-m-r10"><svg width="162" height="148" viewBox="0 0 162 148" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M72.8286 112.017V18.6504" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<path d="M53.1658 29.4181L67.7911 13.4001C70.4963 10.4374 75.1617 10.4374 77.8668 13.4002L92.4919 29.4181" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<path d="M43.874 112.017L29.2487 128.035C26.5435 130.997 21.8781 130.997 19.173 128.035L4.54788 112.017" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<path d="M116.352 18.6503L137.027 12.091C140.851 10.8778 144.892 13.2105 145.753 17.1289L150.41 38.3134" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<path d="M72.9697 81.6625H109.942C112.79 81.6625 115.339 79.8924 116.334 77.2231L138.765 17.042" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<path d="M67.6826 60.5139H31.0326C27.265 60.5139 24.2107 63.5682 24.2107 67.3358V122.785" stroke="#C5FA31" stroke-width="9.09587" stroke-linecap="round"/>
<circle cx="72.9697" cy="129.749" r="13.1843" stroke="#C6F932" stroke-width="9.09587"/>
</svg>
</span>USE CASES</span>
        <ul class="rdcl-gen-use-list rdc-m-0 rdc-m-b10">
          ${useCases.map((useCase) => `<li>${useCase}</li>`).join("")}
        </ul>
      </div>
    </div>
  `;
}

function renderUseCta(cta) {
  if (!cta) return "";

  return `
    <div class="rdcl-gen-co-cta">
      <div class="rdcl-gen-co-cta-inner rdc-d-flex rdc-ai-cen rdc-p-10 rdc-lab-border-3 rdc-bg-dark-7">
        <span class="rdcl-gen-co-cta-icon rdc-m-r10 rdc-d-flex">${cta.icon}</span>
        <span class="rdcl-gen-co-cta-text rdc-ff-baij">${cta.text}</span>
      </div>
    </div>
  `;
}

function insertSettings(content) {
  const container = document.querySelector(".rdcl-gen--cols-set");

  if (!container) return;

  container.innerHTML = `
    <div class="rdcl-gen--cols-set-inner">
      <div class="rdc-d-flex rdc-ff-col">
        ${content.sections.map(renderSettingsSection).join("")}
        ${content.actions.map(renderAction).join("")}
        ${renderCssDownloads(content)}
      </div>
    </div>
  `;
}

function renderCssDownloads(content) {
  return [content.generatorCssDownload, content.globalCssDownload]
    .map(renderCssDownload)
    .join("");
}

function renderCssDownload(download) {
  if (!download || !download.href) return "";

  return `
    <a href="${download.href}" class="rdcl-copy-css-btn rdc-d-none rdc-fs-12 rdc-c-white-2 rdc-jc-cen rdc-m-t10 rdc-ff-baij" download="${download.fileName || ""}">
      ${download.labelHtml || "Download CSS File"}
    </a>
  `;
}

function insertPreview(content) {
  const container = document.querySelector(".rdcl-gen--layout-vw");

  if (!container) return;

  container.innerHTML = `<div class="${content.wrapperClass}" id="${content.stageId}"></div>`;
}

function renderSettingsSection(section) {
  return `
    ${section.title ? `<h4>${section.title}</h4>` : ""}
    ${section.fields.map(renderField).join("")}
  `;
}

function renderField(field) {
  return `
    <div class="rdc-d-flex">
      <label class="rdc-w-half">${field.label}</label>
      <input
        class="${field.className} rdcl-gen--cols-field rdc-w-half"
        type="${field.type}"
        ${field.value ? `value="${field.value}"` : ""}
        ${field.min ? `min="${field.min}"` : ""}
        ${field.max ? `max="${field.max}"` : ""}
      />
    </div>
  `;
}

function renderAction(action) {
  return `<button id="${action.id}" type="${action.type}">${action.label}</button>`;
}

function insertTutorial(content) {
  const container = document.querySelector(".rdcl-gen-tutorial");

  if (!container || !content) return;

  container.innerHTML = renderTutorial(content);
}

function renderTutorial(content) {
  return `
    <div class="rdcl-gen-tutorial-inner">
      <div class="rdcl-gen-tutorial-project">${content.projectPreview}</div>
      <div class="rdcl-cols-main">
        <div class="rdcl-cols-inner rdc-d-flex rdc-m-fw-wrap">
          <div class="rdcl-cols-col rdc-w-half rdc-m-w-full">${content.video}</div>
          <div class="rdcl-cols-col rdc-w-half rdc-m-w-full">${content.text}</div>
        </div>
      </div>
    </div>
  `;
}

function insertSmallCta(content) {
  const container = document.querySelector(".rdcl-gen-sm-cta");

  if (!container || !content) return;

  container.innerHTML = renderSmallCta(content);
}

function renderSmallCta(content) {
  return `
    <div class="rdcl-gen-sm-cta-inner">
      <div class="rdcl-cols-main">
        <div class="rdcl-cols-inner rdc-d-flex rdc-m-fw-wrap">
          <div class="rdcl-cols-col rdc-w-half rdc-m-w-full rdc-d-flex rdc-ai-cen">
            <span class="rdcl-gen-sm-cta-icon rdc-d-flex rdc-m-r10">${content.icon}</span>
            <span>${content.text}</span>
          </div>
          <div class="rdcl-cols-col rdc-w-half rdc-m-w-full rdc-d-flex rdc-jc-fle rdc-ai-cen">
            <a class="rdc-lab-5th-btn rdc-lab-btn-fit" href="${content.button.href}">${content.button.label}</a>
          </div>
        </div>
      </div>
    </div>
  `;
}

function bindColumnLayoutGenerator(content) {
  const output = document.querySelector(`#${content.leftView.output.codeId}`);
  const stage = document.querySelector(`#${content.preview.stageId}`);
  const copyButton = document.querySelector("#rdcl-copy-btn");
  const controls = document.querySelectorAll(".rdcl-gen--cols-field");
  let latestColumnLayoutOutput = "";
  let previewStyle = document.getElementById("cols-preview-style");

  if (!previewStyle) {
    previewStyle = document.createElement("style");
    previewStyle.id = "cols-preview-style";
    document.head.appendChild(previewStyle);
  }

  if (!output || !stage || !copyButton) return;

  const update = (event) => {
    handleExclusiveColumnFields(event ? event.target : null);
    const code = buildColumnLayoutOutput();

    latestColumnLayoutOutput = code;
    stage.innerHTML = getColumnLayoutPreviewHtml(code);
    previewStyle.textContent = getColumnLayoutPreviewCss(code);
    output.textContent = code;
  };

  controls.forEach((control) => {
    control.addEventListener("input", update);
    control.addEventListener("change", update);
  });

  const copyOutputCode = async (event) => {
    event.preventDefault();
    update();
    const didCopy = await copyToClipboard(latestColumnLayoutOutput);

    copyButton.textContent = didCopy ? "Copied!" : "Copy Failed";
    setTimeout(() => {
      copyButton.textContent = "Copy Code";
    }, 1500);
  };

  copyButton.addEventListener("click", copyOutputCode);

  update();
}

function getColumnLayoutPreviewHtml(code) {
  return String(code).replace(/<style>[\s\S]*?<\/style>\s*/i, "");
}

function getColumnLayoutPreviewCss(code) {
  const match = String(code).match(/<style>([\s\S]*?)<\/style>/i);

  return match ? match[1] : "";
}

function handleExclusiveColumnFields(input) {
  if (!input || !input.value) return;

  const exclusiveFields = {
    "rdcl-gen--cols-perrow": ".rdcl-gen--cols-scrwidth",
    "rdcl-gen--cols-scrwidth": ".rdcl-gen--cols-perrow",
    "rdcl-gen--cols-perrowtab": ".rdcl-gen--cols-scrwidthtab",
    "rdcl-gen--cols-scrwidthtab": ".rdcl-gen--cols-perrowtab",
    "rdcl-gen--cols-perrowmob": ".rdcl-gen--cols-scrwidthmob",
    "rdcl-gen--cols-scrwidthmob": ".rdcl-gen--cols-perrowmob",
  };

  Object.entries(exclusiveFields).some(([className, oppositeSelector]) => {
    if (!input.classList.contains(className)) return false;

    const opposite = document.querySelector(oppositeSelector);
    if (opposite) opposite.value = "";
    return true;
  });
}

function buildColumnLayoutOutput() {
  const values = getColumnValues();
  const containerClasses = [];
  const mainClasses = [];
  const dynamicCss = [];

  if (values.perRow) {
    containerClasses.push("rdc-fw-wrap");
  }

  if (values.scrollWidth) {
    containerClasses.push("rdc-w-fit");
    mainClasses.push("rdc-ofx-scr");
    dynamicCss.push(`.rlab-cols-col {
  width: ${values.scrollWidth}px;
}`);
  }

  if (values.perRowTablet) {
    containerClasses.push("rdc-t-w-full", "rdc-t-fw-wrap");
  }

  if (values.scrollWidthTablet) {
    containerClasses.push("rdc-t-w-fit", "rdc-t-fw-nwra");
    mainClasses.push("rdc-t-ofx-scr");
    dynamicCss.push(`@media only screen and (max-width: 1024px) {
  .rlab-cols-col {
    width: ${values.scrollWidthTablet}px;
  }
}`);
  }

  if (values.perRowMobile) {
    containerClasses.push("rdc-m-w-full", "rdc-m-fw-wrap");
  }

  if (values.scrollWidthMobile) {
    containerClasses.push("rdc-m-w-fit", "rdc-m-fw-nwra");
    mainClasses.push("rdc-m-ofx-scr");
    dynamicCss.push(`@media only screen and (max-width: 767px) {
  .rlab-cols-col {
    width: ${values.scrollWidthMobile}px;
  }
}`);
  }

  const widthDesktop = values.scrollWidth
    ? ""
    : getWidthClass(values.perRow || values.columnCount);
  const widthTablet = values.scrollWidthTablet
    ? ""
    : getWidthClass(values.perRowTablet, "t");
  const widthMobile = values.scrollWidthMobile
    ? ""
    : getWidthClass(values.perRowMobile, "m");
  const columnClasses = [
    "rlab-cols-col",
    widthDesktop,
    widthTablet,
    widthMobile,
  ].filter(Boolean);
  const columns = Array.from({ length: values.columnCount }, () => {
    return `    <div class="${columnClasses.join(" ")}"></div>`;
  }).join("\n");
  const mainClassList = ["rlab-cols-main", ...mainClasses].filter(Boolean);
  const innerClassList = [
    "rlab-cols-inner",
    "rdc-d-flex",
    ...containerClasses,
  ].filter(Boolean);
  const html = `<div class="${mainClassList.join(" ")}">
  <div class="${innerClassList.join(" ")}">
${columns}
  </div>
</div>`;
  const usedClasses = [...mainClassList, ...innerClassList, ...columnClasses];
  const css = buildColumnLayoutCss(usedClasses, dynamicCss);

  return `<style>
${css}
</style>

${html}`;
}

function buildColumnLayoutCss(usedClasses, dynamicCss = []) {
  const uniqueClasses = [...new Set(usedClasses.filter(Boolean))];
  const baseCss = [
    `* {
  box-sizing: border-box;
}`,
    ...getCssRulesForClasses(uniqueClasses, columnCssRules.base),
  ];
  const tabletCss = getCssRulesForClasses(uniqueClasses, columnCssRules.tablet);
  const mobileCss = getCssRulesForClasses(uniqueClasses, columnCssRules.mobile);

  if (tabletCss.length) {
    baseCss.push(`@media only screen and (max-width: 1024px) {
${indentCss(tabletCss.join("\n\n"))}
}`);
  }

  if (mobileCss.length) {
    baseCss.push(`@media only screen and (max-width: 767px) {
${indentCss(mobileCss.join("\n\n"))}
}`);
  }

  return [...baseCss, ...dynamicCss].filter(Boolean).join("\n\n");
}

function getCssRulesForClasses(classNames, ruleMap) {
  return classNames.map((className) => ruleMap[className]).filter(Boolean);
}

function indentCss(css) {
  return css
    .split("\n")
    .map((line) => (line ? `  ${line}` : line))
    .join("\n");
}

const columnCssRules = {
  base: {
    "rlab-cols-main": `.rlab-cols-main {
  width: 100%;
}`,
    "rlab-cols-inner": `.rlab-cols-inner {
  width: 100%;
}`,
    "rlab-cols-col": `.rlab-cols-col {
  box-sizing: border-box;
}`,
    "rdc-d-flex": `.rdc-d-flex {
  display: flex;
}`,
    "rdc-fw-wrap": `.rdc-fw-wrap {
  flex-wrap: wrap;
}`,
    "rdc-w-fit": `.rdc-w-fit {
  width: fit-content;
}`,
    "rdc-ofx-scr": `.rdc-ofx-scr {
  overflow-x: scroll;
}`,
    ...createWidthCssRules("rdc-w-"),
  },
  tablet: {
    "rdc-t-fw-wrap": `.rdc-t-fw-wrap {
  flex-wrap: wrap;
}`,
    "rdc-t-fw-nwra": `.rdc-t-fw-nwra {
  flex-wrap: nowrap;
}`,
    "rdc-t-w-fit": `.rdc-t-w-fit {
  width: fit-content;
}`,
    "rdc-t-ofx-scr": `.rdc-t-ofx-scr {
  overflow-x: scroll;
}`,
    ...createWidthCssRules("rdc-t-w-"),
  },
  mobile: {
    "rdc-m-fw-wrap": `.rdc-m-fw-wrap {
  flex-wrap: wrap;
}`,
    "rdc-m-fw-nwra": `.rdc-m-fw-nwra {
  flex-wrap: nowrap;
}`,
    "rdc-m-w-fit": `.rdc-m-w-fit {
  width: fit-content;
}`,
    "rdc-m-ofx-scr": `.rdc-m-ofx-scr {
  overflow-x: scroll;
}`,
    ...createWidthCssRules("rdc-m-w-"),
  },
};

function getColumnValues() {
  return {
    columnCount: getIntegerValue(".rdcl-gen--cols-count", 2),
    perRow: getIntegerValue(".rdcl-gen--cols-perrow", 0),
    scrollWidth: getIntegerValue(".rdcl-gen--cols-scrwidth", 0),
    perRowTablet: getIntegerValue(".rdcl-gen--cols-perrowtab", 0),
    scrollWidthTablet: getIntegerValue(".rdcl-gen--cols-scrwidthtab", 0),
    perRowMobile: getIntegerValue(".rdcl-gen--cols-perrowmob", 0),
    scrollWidthMobile: getIntegerValue(".rdcl-gen--cols-scrwidthmob", 0),
  };
}

function getWidthClass(columns, prefix = "") {
  const widthKey = getWidthKey(columns);

  if (!widthKey) return "";

  return `rdc-${prefix ? `${prefix}-` : ""}w-${widthKey}`;
}

function getWidthKey(columns) {
  const widths = {
    1: "full",
    2: "half",
    3: "33-33",
    4: "25",
    5: "20",
    6: "16-67",
    7: "14-29",
    8: "12-5",
    9: "11-11",
    10: "10",
  };

  return widths[columns] || "";
}

function createWidthCssRules(prefix) {
  const widths = {
    10: "10%",
    "11-11": "11.11%",
    "12-5": "12.5%",
    "14-29": "14.29%",
    "16-67": "16.67%",
    20: "20%",
    25: "25%",
    "33-33": "33.33%",
    half: "50%",
    "66-67": "66.67%",
    75: "75%",
    80: "80%",
    full: "100%",
  };

  return Object.fromEntries(
    Object.entries(widths).map(([key, value]) => {
      const className = `${prefix}${key}`;

      return [
        className,
        `.${className} {
  width: ${value};
}`,
      ];
    }),
  );
}

function getIntegerValue(selector, fallback) {
  const control = document.querySelector(selector);
  const value = Number.parseInt(control ? control.value : "", 10);

  return Number.isFinite(value) ? value : fallback;
}

function copyToClipboard(value) {
  if (navigator.clipboard) {
    return navigator.clipboard
      .writeText(value)
      .then(() => true)
      .catch(() => copyToClipboardFallback(value));
  }

  return copyToClipboardFallback(value);
}

function copyToClipboardFallback(value) {
  const textarea = document.createElement("textarea");
  textarea.value = value;
  textarea.setAttribute("readonly", "");
  textarea.style.position = "fixed";
  textarea.style.opacity = "0";
  document.body.appendChild(textarea);
  textarea.select();
  let didCopy = false;

  try {
    didCopy = document.execCommand("copy");
  } catch (error) {
    didCopy = false;
  }

  textarea.remove();

  return didCopy;
}

function escapeHtml(value) {
  return String(value)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;");
}
