const generatorContent = {
  leftView: {
    ...
    useCases: [
      "Add countdown bars for sitewide sales and campaigns",
      "Promote limited-time offers, bundles, and launches",
      "Count down to launches, restocks, or store events",
      "Add urgency messaging to product or landing pages",
    ],
    useCta: {
...
`,
    ...
    sections: [
      ...
      },
      {
        ...
        fields: [
          ...
          { type: "datetime-local", label: "End Date", id: "timer-end-date" },
        ],
      },
      {
        title: "Radius",
  ...
  previewStageId: "timerPreviewStage",
  tutorial: {
...
initTimerBarGenerator(generatorContent);

function initTimerBarGenerator(content) {
  const init = () => {
    const view = document.querySelector(".rdcl-gen--cols-vw");
    const settings = document.querySelector(".rdcl-gen--cols-set");
    const preview = document.querySelector(".rdcl-gen--layout-vw");
    ...
    view.innerHTML = renderLeftView(content.leftView);
    settings.innerHTML = renderSettings(content.settings);
    preview.innerHTML = `<div class="rdcl-gen--layout-vw-inner" id="${content.previewStageId}"></div>`;
...
function renderLeftView(content) {
  return `<div class="rdcl-gen--cols-vw-inner">
...
function renderSettings(content) {
  return `<div class="rdcl-gen--cols-set-inner">
...
function renderSection(section) {
  const note =
    section.title === "Content"
      ? `<p class="rdcl-gen-note rdc-fs-12 rdc-c-white-2 rdc-m-b10">Note: Use UTC time for timer dates.</p>`
      : "";

  return `${note}<h4>${section.title}</h4>${section.fields.map(renderField).join("")}`;
}

function renderField(field) {
  return `<div class="rdc-d-flex">
    <label class="rdc-w-half" for="${field.id}">${field.label}</label>
    <input type="${field.type}" id="${field.id}" class="rdcl-gen--cols-field rdc-w-half"${field.placeholder ? ` placeholder="${field.placeholder}"` : ""} />
  </div>`;
}
