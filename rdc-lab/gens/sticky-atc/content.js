const generatorContent = {
  leftView: {
    title: "Sticky Add To Cart Generator",
    description:
      "Generate a Shopify-focused sticky add to cart bar with product title, price, variants, quantity, and purchase controls. Keep buying actions visible on long product pages and refine with AI prompts.",
    callouts: [
      {
    ...
    useCases: [
      "Add a sticky add to cart bar to Shopify product pages",
      "Keep purchase controls visible on long landing pages",
    ],
    useCta: {
...
`,
      text: "Add conversion-focused sticky purchase bars without extra apps.",
    },
    output: {
      element: "pre",
      codeId: "satcOutput",
  ...
  settings: {
    sections: [
      {
        title: "Behavior",
        notes: [
          "Stays fixed at the bottom of the screen once shown.",
          "Can be configured to appear automatically after the user scrolls past a certain height.",
          "Keeps product title and price visible while purchase controls stay accessible.",
          "On tablet and mobile, the variant selector, quantity, and add to cart button move to the next row for better space.",
          "Copied code includes sample JavaScript so the sticky bar can be triggered after a chosen scroll position.",
        ],
      },
    ],
    actions: [
    ...
    generatorCssDownload: {
      href: "",
      fileName: "sticky-add-to-cart-generator.css",
...
`,
  ...
  preview: {
    ...
    stageId: "satcPreviewStage",
  },
};

initStickyAddToCartGenerator(generatorContent);

function initStickyAddToCartGenerator(content) {
  const init = () => {
    const hasTemplate =
...
function renderCallout(callout) {
  return `<span class="rdcl-gen-co-item rdc-lab-border-4 rdc-p-5 rdc-m-r5 rdc-bg-dark-6 rdc-d-iflex rdc-ai-cen"><span class="rdcl-gen-co-icon rdc-d-flex rdc-m-r5">${callout.icon}</span><span class="rdcl-gen-co-text rdc-fs-10 rdc-tt-upc rdc-ff-baij rdc-lh-1">${callout.text}</span></span>`;
}

function renderUseCases(useCases = []) {
  if (!useCases.length) return "";
  return `<div class="rdcl-gen-use rdc-m-bl20"><div class="rdcl-gen-use-inner rdc-lab-border rdc-p-20 rdc-bg-dark-6"><span class="rdc-ff-baij rdc-fw-500 rdc-c-white-3 rdc-d-flex rdc-ai-cen">USE CASES</span><ul class="rdcl-gen-use-list rdc-m-0 rdc-m-b10">${useCases.map((useCase) => `<li>${useCase}</li>`).join("")}</ul></div></div>`;
}
...
function renderCssDownload(download) {
  ...
  return `<a href="${download.href}" class="rdcl-copy-css-btn rdc-d-none rdc-fs-12 rdc-c-white-2 rdc-jc-cen rdc-m-t10 rdc-ff-baij" download="${download.fileName || ""}">${download.labelHtml || "Download CSS File"}</a>`;
}
