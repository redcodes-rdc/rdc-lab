(function () {
  ...
  function updateCount(visibleCount, totalCount) {
    ...
    const label = visibleCount === 1 ? "tutorial" : "tutorials";
  ...
  function applyStateToControls() {
    ...
    const searchInput = document.querySelector(selectors.search);
    if (searchInput && normalizeSearch(searchInput.value) !== state.search) {
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initTutorialCollection);
    return;
  }

  initTutorialCollection();
})();
