                ...
                <div class="rdcl-ai-panel rdcl-ai-preview-panel">
                  <div class="rdcl-ai-panel-title">
                    ...
                    <button class="rdcl-ai-preview-toggle" type="button" data-rdcl-ai-preview-toggle aria-expanded="true">
                      ...
                      <span class="rdcl-ai-preview-toggle-text">Hide output</span>
                    </button>
                  </div>
...
function initRdcAiRest() {
  ...
  previewToggle?.addEventListener("click", () => {
    ...
    if (previewToggleText) {
      previewToggleText.textContent = isHidden ? "Show output" : "Hide output";
    }
  ...
  function buildRdcAiPromptText() {
    ...
    return [
      ...
      "- Enhance the Generated Code at the bottom using the selected tasks, generator configuration, context, design options, and additional notes.",
      "- Return the enhanced code itself, not advice about how to enhance it.",
      "- Treat the generated code as the working baseline. Improve it without rebuilding from scratch unless a selected task clearly requires a different implementation.",
      "",
      "## Instruction Hierarchy",
      "When instructions appear to conflict, follow the higher-priority rule first:",
      "1. Preserve existing working functionality, user intent, and compatibility-sensitive public hooks.",
      "2. Complete every selected task.",
      "3. Follow task-specific platform requirements when a selected task changes the target platform or output format.",
      "4. Preserve the existing implementation whenever it remains compatible with the selected tasks.",
      "5. Improve maintainability, accessibility, performance, and production readiness within the selected task scope.",
      "6. Apply optional polish from Context, Design Options, task options, and Additional Notes.",
      "",
      "## Global Rules",
      "- First evaluate the generated code and identify what each selected task actually requires.",
      "- Keep changes scoped to the selected tasks. Do not add unrelated redesigns, features, libraries, frameworks, fonts, assets, schema, platform code, or routing.",
      "- Prefer preserving existing implementations. Replace them only when a selected task clearly benefits from a different implementation.",
      "- If a selected task is already satisfied, leave that part unchanged and state that no change was necessary in the Implementation Summary.",
      ...
      "",
      "## Compatibility Rules",
      "- Protect compatibility-sensitive items: existing behavior, public API, user-facing content, existing rdc-* and rlab-* classes, IDs, data attributes, JavaScript hooks, event hooks, ARIA relationships, form behavior, and generator output intent.",
      "- Modify compatibility-sensitive items only when a selected task requires it. Explain each compatibility-sensitive change in the Implementation Summary.",
      "- Internal implementation details may change when useful for a selected task, including HTML structure, CSS organization, selector strategy, JavaScript implementation, comments, and internal helper names.",
      "- Keep CSS scoped to the component. Keep JavaScript safe for multiple component instances with scoped initialization, no duplicate event listeners, no unnecessary globals, and no interference with other page scripts.",
      "",
      "## Task Scope Rules",
      "- Each selected task controls only its own responsibility. Do not let one task introduce unrelated changes from another task.",
      "- Design improves visual polish without changing behavior unless needed for the design request.",
      "- Mobile and responsive work improves small-screen usability without redesigning desktop behavior unless needed to prevent breakage.",
      "- Performance removes practical bloat or inefficiency without changing visual output or behavior unless required.",
      "- Accessibility improves semantics, keyboard support, focus, ARIA, contrast, and reduced-motion behavior without restyling unrelated areas.",
      "- SEO improves semantic and metadata-related output only where relevant to the component and provided content.",
      "- Animations add motion only when selected, and must respect accessibility and reduced-motion needs.",
      "- Platform conversion tasks may change markup syntax, structure, schema, file organization, and dynamic bindings as needed for the target platform while preserving the component's intent and behavior.",
      "",
      "## Multi-Task Precedence",
      "- Platform conversion requirements take precedence over preserving exact HTML structure because target platforms may require templates, schema, blocks, slots, components, props, or bindings.",
      "- Accessibility, security, and functional correctness take precedence over visual preservation when they are necessary for the selected task.",
      "- Responsive fixes may adjust layout structure when needed to prevent overflow, broken touch behavior, or unusable mobile layouts.",
      "- Performance changes should not redesign the component unless the selected performance task cannot be completed safely without structural cleanup.",
      "- Design and animation polish have the lowest precedence and must not break functionality, accessibility, responsive behavior, or platform requirements.",
      "",
      ...optionalContextSection,
      "## Generator Configuration",
      "- Use these current generator settings as design and behavior context. Preserve or build on them unless a selected task requires a compatible change.",
      ...
      "- Before responding, silently verify: every selected task was evaluated; functionality still works; compatibility-sensitive hooks are preserved or justified; JavaScript selectors match the returned HTML; CSS remains scoped; multiple instances remain supported; platform conversion was only performed when selected; accessibility changes are appropriate; no unrelated redesigns were introduced; the code is complete and production-ready.",
  ...
  return [
    {
      ...
      description: "Keep compatible structure and rdc-* / rlab-* classes.",
      defaultSelected: true,
      prompt: [
        "Instruction:",
        "- Preserve the generated component as the foundation for all selected improvements whenever compatible with the selected tasks.",
        "",
        "Specific Requirements:",
        "- Preserve compatibility-sensitive items: existing rdc-* and rlab-* classes, IDs, data attributes, JavaScript hooks, event hooks, ARIA relationships, user-facing content, and working behavior.",
        "- Preserve existing structure and selectors when practical, but change them when a selected task clearly requires or benefits from it.",
        "- If any compatibility-sensitive item must change, explain exactly why under Implementation Summary > Preserve HTML & Class Naming.",
      ].join("\n"),
    },
    {
      id: "design",
